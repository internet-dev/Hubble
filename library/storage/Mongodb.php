...
class Mongodb
{
     ...
     * 失败：false
     */
    public function __construct($connect = true, $auto_balance = true)
    {
        $cfg = \Hubble\Config\Config::get('mongodb');
        $mongo_server = $cfg['server'];
        //\Hubble\Library\Log::debug('$mongo_server: ' . $mongo_server);
        ...
        try {
            $this->mongo = new \MongoClient ($mongo_server_str, array(
                'connect' => $connect,
            ));
     ...
     * 返回值：表的记录数
     */
    public function count($table_name, $query_condition)
    {
        $dbname = $this->curr_db_name;
        return $this->mongo->$dbname->$table_name->count($query_condition);
    }
    ...
    public function close()
    {
        try {
            ...
            \Hubble\Library\Log::error($e->getMessage());
        }
    }
